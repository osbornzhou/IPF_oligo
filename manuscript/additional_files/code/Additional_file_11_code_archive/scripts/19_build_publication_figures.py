...
def make_workflow_figure() -> None:
    ...
    boxes = [
        ...
        ("pathway", 0.14, 0.20, 0.16, "Pathway and PPI\nGO/KEGG/Reactome\nSTRING hubs"),
        ("ml", 0.40, 0.20, 0.16, f"Machine learning\nDiscovery-only features\n{int(ml_qc['final_panel_size'])}-gene panel"),
        ("sc", 0.66, 0.20, 0.18, f"Single-cell localization\n{sc_cells:,} matched cells\ncell-type delta estimates"),
    ]
    box_lookup = {}
    ...
    for key in ["pathway", "ml", "sc"]:
        target = box_lookup[key]
        arrow((robust["cx"], robust["bottom"]), (target["cx"], target["top"]))
    ax.text(0.5, 0.94, "Study workflow and evidence layers", ha="center", va="center", fontsize=12, fontweight="bold")
    ax.text(0.5, 0.05, "All downstream evidence layers use QC-passed datasets and preserve external validation cohorts for model testing.", ha="center", fontsize=8)
...
Figure 1. Overview of the multi-cohort analysis workflow. Public GEO mRNA, miRNA, and single-cell datasets were curated, quality controlled, and analyzed through differential expression, cross-cohort robust candidate screening, miRNA-mRNA target-axis integration, enrichment and STRING PPI analysis, leakage-controlled machine learning, published-signature comparison, and single-cell localization.
...
Figure 5. Machine-learning validation and benchmarking. (A) External ROC curves for the best discovery-only model. (B) External precision-recall curves for the same model. (C) Mean external ROC AUC of the proposed model compared with published IPF signatures evaluated under the same train/external-validation framework. Comparator results reflect retrained reported gene sets under a harmonized framework, not direct reuse of original published models. (D) Top 20 features ranked by selection frequency across resampling and model families.

Figure 6. Single-cell validation of prioritized targets. (A) IPF-control expression differences for selected target genes across representative disease-relevant cell populations. (B) Largest cell-type-level target shifts after filtering non-informative cell labels.
"""
