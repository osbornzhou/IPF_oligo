...
This folder contains the numbered scripts used for the submitted IPF boundary-tested transcriptomic perturbation-triage study. The authoritative current script order is also recorded in:
...
| `26_oligonucleotide_actionability_index.py` | Build the oligonucleotide-focused perturbation-triage validation-planning score and triage-map figure. |
| `27_single_cell_pseudobulk_validation.py` | Run donor-aware pseudobulk validation for core candidates. |
| `29_actionability_weight_sensitivity.py` | Run perturbation-triage score weight-sensitivity analyses. |
| `30_mirna_exact_axis_support_table.py` | Build exact hsa-miR-375 axis support and cross-check tables. |
| `31_build_pseudobulk_figure.py` | Build high-resolution donor-aware pseudobulk figure panel. |
...
| `37_build_editable_main_figures.py` | Build editable SVG source files for main Figures 1-6. |
